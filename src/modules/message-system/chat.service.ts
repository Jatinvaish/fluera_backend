// ============================================
// src/modules/message-system/chat.service.ts - ENHANCED WITH OPTIMIZATIONS
// ============================================
import {
  Injectable,
  NotFoundException,
...
export class ChatService {
  ...
  // ==================== ULTRA-FAST MESSAGE SENDING (OPTIMIZED) ====================

  /**
   * ✅ Send message in <200ms with parallel validation and caching
   */
  async sendMessage(dto: SendMessageDto, userId: number, tenantId: number) {
    ...
    try {
      ...
      if (!dto.encryptedContent || !dto.encryptionIv || !dto.encryptionAuthTag) {
        throw new BadRequestException('Message must be encrypted with AES-256-GCM');
      }
      ...
      const channelVersion = channelInfo.encryption_version || 'v1';
      ...
      const message = await this.sqlService.query(
        ...
        `,
        {
          ...
          senderKeyVersion: senderKey.key_version,
      ...
      // ✅ STEP 4: Cache invalidation (async, non-blocking)
  ...
  /**
   * ✅ Fetch multiple user keys in ONE query (BULK OPTIMIZATION)
   */
  ...
  async getMessages(channelId: number, userId: number, dto: GetMessagesDto) {
    ...
    try {
      const cached = await this.redisService.get(cacheKey);
      if (cached) {
        this.logger.log(`✅ Messages from cache in ${Date.now() - startTime}ms`);
        return JSON.parse(cached);
      }
    } catch (error) {
    ...
    this.redisService.set(cacheKey, JSON.stringify(result), 30).catch(() => {});
  ...
  private validateEncryptionPayload(dto: SendMessageDto): void {
    ...
    if (dto.encryptionIv.length !== 24) {
      throw new BadRequestException('encryptionIv must be 16 bytes (24 base64 chars)');
    }

    if (dto.encryptionAuthTag.length !== 24) {
      throw new BadRequestException('encryptionAuthTag must be 16 bytes (24 base64 chars)');
    }
  }
  ...
  async getUserChannels(userId: number, tenantId: number, dto: GetChannelsDto) {
    ...
    try {
      ...
      this.logger.warn('Redis unavailable for channel list');
    }
  ...
  async addChannelMembers(channelId: number, dto: AddChannelMembersDto, userId: number) {
    ...
    const channelKeyResult = await this.sqlService.query(
      `SELECT key_material_encrypted FROM chat_channel_keys
       WHERE channel_id = @channelId AND status = 'active'`,
      { channelId },
    );

    if (channelKeyResult.length === 0) {
      throw new BadRequestException('Channel encryption key not found');
    }

    // Decrypt channel key with master key
    ...
    // Get all user keys in bulk
    const userKeys = await this.getUserKeysBulk(dto.userIds.map(Number));

    for (const memberId of dto.userIds) {
      try {
        const userKey = userKeys.get(Number(memberId));
        if (!userKey) {
          this.logger.warn(`User ${memberId} has no encryption key`);
          continue;
        }
        ...
        const result = await this.sqlService.query(
          ...
          VALUES (
            ...
            1, GETUTCDATE(), @userId, GETUTCDATE()
          )`,
          {
            channelId,
            tenantId,
  ...
  async bulkMarkAsRead(channelId: number, messageIds: number[], userId: number) {
    await this.checkChannelMembership(channelId, userId);
  ...
  async getUnreadCount(userId: number, tenantId: number) {
    ...
    try {
      ...
      this.logger.warn('Redis unavailable for unread count');
    }
    ...
    await this.redisService.set(cacheKey, JSON.stringify(result[0]), 30).catch(() => {});
  ...
  async getMessageStatus(messageId: number, userId: number) {
    ...
    const totalRecipients = await this.sqlService.query(
      `SELECT COUNT(*) as count FROM chat_participants 
       ...
       AND is_active = 1`,
      { channelId: message[0].channel_id, senderId: message[0].sender_user_id },
    );
  ...
  async createDirectMessage(dto: CreateDirectMessageDto, userId: number, tenantId: number) {
      ...
      const newChannel = await this.createChannel(
        {
          ...
          isPrivate: true,
          memberIds: [Number(dto.recipientUserId)],
        },
        userId,
        tenantId,
      );
  ...
  async searchMessages(userId: number, tenantId: number, dto: SearchMessagesDto) {
    let query = `
      SELECT m.*,
             u.first_name as sender_first_name,
             u.last_name as sender_last_name,
  ...
  async rotateChannelKey(channelId: number, reason: string, userId: number): Promise<any> {
    const rotationId = crypto.randomUUID();

    try {
      this.logger.log(`[${rotationId}] Starting channel key rotation for channel ${channelId}`);
      ...
      const userKeys = await this.getUserKeysBulk(userIds);

      let updatedCount = 0;
      for (const participant of participants) {
        try {
          const userKey = userKeys.get(participant.user_id);
          ...
          this.logger.warn(`Failed to update key for user ${participant.user_id}: ${error.message}`);
        }
      }

      // Store new master-encrypted key
      await this.sqlService.query(
        `UPDATE chat_channel_keys
         SET status = 'rotated', updated_at = GETUTCDATE()
         WHERE channel_id = @channelId AND status = 'active'`,
        { channelId },
      );
      ...
      await this.sqlService.query(
        ...
        {
          channelId,
          keyMaterial: await this.encryptionService.encryptWithMasterKey(newChannelKey),
          fingerprint: this.encryptionService.calculateKeyFingerprint(newChannelKey),
          keyVersion: currentVersionNum + 1,
          userId,
        },
      );
      ...
      this.logger.log(`[${rotationId}] ✅ Key rotated from ${currentVersion} to ${newVersion}`);
      ...
      this.logger.error(`[${rotationId}] Key rotation failed: ${error.message}`);
      throw error;
    }
  }
  ...
  async adminDecryptMessage(
    ...
    try {
      ...
      this.logger.log(`✅ Admin ${adminUserId} successfully decrypted message ${messageId}`);
      ...
      throw new BadRequestException(`Failed to decrypt message: ${error.message}`);
    }
  }
  ...
  async backfillChannelKeys(adminUserId: number): Promise<{
    ...
    try {
      ...
      for (const channel of channelsResult) {
        try {
          ...
          if (!userKey || !userKey[0].backup_encrypted_private_key) {
            stats.failed++;
            stats.errors.push(`Channel ${channel.id}: User backup key not found`);
            continue;
          }
          ...
          const keyVersion = parseInt((channel.encryption_version || 'v1').substring(1));
          ...
          await this.sqlService.query(
            ...
            VALUES (
              @channelId, @keyMaterial, @fingerprint,
              'AES-256-GCM', @keyVersion, 'active', GETUTCDATE(), @adminUserId, GETUTCDATE()
            )`,
            {
              channelId: channel.id,
              keyMaterial: await this.encryptionService.encryptWithMasterKey(channelKey),
              fingerprint: this.encryptionService.calculateKeyFingerprint(channelKey),
              keyVersion,
              adminUserId,
            },
          );
      ...
      this.logger.log(`✅ Backfill completed: ${stats.successful}/${stats.processed} successful`);
      ...
      try {
        ...
        await this.sqlService.query(
          `INSERT INTO message_read_receipts (message_id, user_id, status, created_at)
           ...
           AND user_id != @userId 
           AND is_active = 1`,
          { messageId: newMessageId, channelId: targetChannelId, userId },
        );
        ...
        this.logger.warn(`Failed to forward to channel ${targetChannelId}:`, error.message);
      }
    }
  ...
  async updateChannelSettings(channelId: number, settings: any, userId: number) {
    await this.checkChannelPermission(channelId, userId, [
      MemberRole.OWNER,
      MemberRole.ADMIN,
    ]);
    ...
    return { message: 'Channel settings updated', settings };
  }
