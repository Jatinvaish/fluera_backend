...
export class ChatService {
  ...
  async getMessage(messageId: number): Promise<EnrichedMessageResponse> {
    ...
    console.log('🔵 [CHAT-SERVICE] Message fetched:', {
    ...
    if (message.attachment_count > 0) {
      console.log('🔵 [CHAT-SERVICE] Fetching attachments for message:', messageId);
      const attachments = await this.sqlService.execute(
        'sp_GetMessageAttachments_Fast',
        { messageId },
      );

      console.log('🔵 [CHAT-SERVICE] Attachments fetched:', attachments?.length || 0);

      if (attachments?.length > 0) {
        console.log('🔵 [CHAT-SERVICE] Generating signed URLs for attachments...');
        message.attachments = await Promise.all(
          attachments.map(async (att: any) => {
            if (att.file_url) {
              const key = att.file_url.replace(/^https?:\/\/[^\/]+\//, '');
              console.log('🔵 [CHAT-SERVICE] Generating signed URL for key:', key);
              const signedUrl = await this.r2Service.getSignedUrl(key);
              console.log('✅ [CHAT-SERVICE] Signed URL generated:', signedUrl);
              return { ...att, url: signedUrl };
            }
            return att;
          }),
        );
        console.log('✅ [CHAT-SERVICE] All attachments with signed URLs:', message.attachments.map(a => ({ id: a.id, url: (a as any).url })));
      }
    }
  ...
  async sendFileMessage(
    ...
    // Get enriched message with signed URLs
    const enrichedMessage = await this.getMessage(message.id);
    console.log('lodo maro - enrichedMessage.attachments:', enrichedMessage.attachments?.map(a => ({ id: a.id, url: (a as any).url })));
  ...
  async sendMultipleFilesAsOneMessage(
    ...
    // Get enriched message with signed URLs already generated by getMessage()
    const enrichedMessage = await this.getMessage(message.id);
    enrichedMessage.attachment_count = attachmentIds.length;
    console.log('✅ [CHAT-SERVICE] sendMultipleFilesAsOneMessage - enrichedMessage.attachments:', enrichedMessage.attachments?.map(a => ({ id: a.id, url: (a as any).url })));
  ...
  async sendMessageWithExistingAttachment(
    ...
    // Get enriched message with signed URLs
    const enrichedMessage = await this.getMessage(message.id);
    console.log('✅ [CHAT-SERVICE] sendMessageWithExistingAttachment - enrichedMessage.attachments:', enrichedMessage.attachments?.map(a => ({ id: a.id, url: (a as any).url })));
