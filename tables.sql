...
-- ============================================
-- ULTRA-FAST CHAT: OPTIMIZED INDEXES & PROCEDURES
-- Run these to achieve <50ms targets
-- ============================================

-- ==================== CRITICAL INDEXES ====================

-- 1. Messages - Covering index for fast list
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_messages_channel_sent_ULTRA')
BEGIN
    CREATE NONCLUSTERED INDEX IX_messages_channel_sent_ULTRA
    ON messages (channel_id, is_deleted, sent_at DESC)
    INCLUDE (id, sender_user_id, message_type, encrypted_content, 
             encryption_iv, encryption_auth_tag, encryption_key_version,
             has_attachments, has_mentions, reply_to_message_id, thread_id,
             is_edited, edited_at)
...
-- 2. Participants - Fast membership + channel list
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_participants_user_tenant_ULTRA')
BEGIN
    CREATE NONCLUSTERED INDEX IX_participants_user_tenant_ULTRA
    ON chat_participants (user_id, is_active, tenant_id)
    INCLUDE (channel_id, role, encrypted_channel_key, last_read_message_id)
    WITH (ONLINE = ON, FILLFACTOR = 90);
END;
GO

-- 3. Channels - Fast stats lookup
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_channels_tenant_activity_ULTRA')
BEGIN
    CREATE NONCLUSTERED INDEX IX_channels_tenant_activity_ULTRA
    ON chat_channels (created_by_tenant_id, is_archived, last_activity_at DESC)
    INCLUDE (id, name, channel_type, is_private, message_count)
    WITH (ONLINE = ON, FILLFACTOR = 90);
END;
GO

-- 4. Reactions - Fast count
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'IX_reactions_message_ULTRA')
BEGIN
    CREATE NONCLUSTERED INDEX IX_reactions_message_ULTRA
...
-- 1. SEND MESSAGE (30ms target)
CREATE OR ALTER PROCEDURE [dbo].[sp_SendMessage_Ultra]
    @channelId BIGINT,
    @userId BIGINT,
    @tenantId BIGINT,
    @messageType NVARCHAR(20),
...
-- 2. GET MESSAGES (25ms target)
CREATE OR ALTER PROCEDURE [dbo].[sp_GetMessages_Ultra]
    @channelId BIGINT,
    @limit INT = 50,
    @beforeId BIGINT = NULL
AS
BEGIN
    ...
    SELECT TOP (@limit)
        ...
        u.last_name,
        u.avatar_url,
        ISNULL((SELECT COUNT(*) FROM message_reactions WITH (NOLOCK) WHERE message_id = m.id), 0) as reaction_count
    FROM messages m WITH (NOLOCK, INDEX(IX_messages_channel_sent_ULTRA))
    INNER JOIN users u WITH (NOLOCK) ON m.sender_user_id = u.id
    WHERE m.channel_id = @channelId
    AND m.is_deleted = 0
    AND (@beforeId IS NULL OR m.id < @beforeId)
...
CREATE OR ALTER PROCEDURE [dbo].[sp_GetUserChannels_Ultra]
    @userId BIGINT,
    @tenantId BIGINT,
    @limit INT = 50
AS
...
UPDATE STATISTICS messages WITH FULLSCAN;
UPDATE STATISTICS chat_participants WITH FULLSCAN;
UPDATE STATISTICS chat_channels WITH FULLSCAN;
GO

PRINT '✅ Ultra-fast indexes and procedures created successfully';
PRINT '🎯 Expected performance:';
PRINT '   - Send message: <30ms';
PRINT '   - List messages: <25ms';
PRINT '   - List channels: <30ms';